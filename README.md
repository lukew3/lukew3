
### Hi there 👋

I'm always building, follow me to see what I build next.

If you like what I do, I'm open to be hired

[📄 See my resume](https://github.com/lukew3/resume/releases/latest/download/lukew3_resume.pdf)

[📧 Email me at lukew25073@gmail.com](mailto:lukew25073@gmail.com)

#### Some things I've made:
* [mathgenerator](https://github.com/lukew3/mathgenerator) - A math problem generator with a large variety of problem types, usable as a python package.
* [stemplayeronline.com](https://beta.stemplayeronline.com) - A website that allows users to mix music using a stem player in their browser.
* [Paint Launcher](https://play.google.com/store/apps/details?id=com.lukew3.paintlauncher) - An Android home screen (launcher) replacement that allows you to launch apps with custom tap zones instead of icons. (Still in testing, join [testers Google Group](https://groups.google.com/g/paint-launcher-testers) to get access)
* [3cards](https://lukew3-3cards.netlify.app) - Flashcards on web3. Runs at 0 cost, allowing for completely free and unblockable education.
* [congol](https://congol.lukew3.com) - An online server for playing a competitive 2 player version of Conway's Game of Life.
* [dropsync](https://dropsync.app) - Perfectly sync a moment in time with a moment in a song.
* [beautifulvhs](https://lukew3.github.io/beautifulvhs) - A 3d collection of blank vhs cases, showcasing a unique style of art from the 1980s and 90s.
* [lukew3/resume](https://github.com/lukew3/resume) - Github Actions CI to build a pdf of my resume from jsonresume and latex template.
* [Color Code Tutor](https://lukew3.github.io/color-code-tutor) - A lightweight game to help you get better at guessing hex color codes.
* [uni2db.com](https://uni2db.com) - An API for public university data like courses offered. Powered by open-source web scrapers.
* [ideaspot](https://github.com/lukew3/ideaspot) - A website where users can share and build each others ideas.
<!--* [ideaspot.org](https://ideaspot.org) - A website where users can share and build each others ideas.-->

![](https://hit.yhype.me/github/profile?user_id=47042841)
